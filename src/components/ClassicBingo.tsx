...
const ClassicBingo: React.FC = () => {
  ...
  const startGame = useCallback(() => {
    ...
    setGameTimer(0);
    setPotentialWinnings(betAmount * 75); // 75x bet for BINGO
  }, [user, betAmount, generateBingoCard, toast]);
  ...
  const checkForBingo = useCallback(() => {
    ...
    if (hasWinner) {
      ...
      toast({
        title: "🎉 BINGO!",
        description: `Congratulations! You won $${potentialWinnings}!`,
        variant: "default"
      });
    }
  }, [bingoCards, potentialWinnings, toast]);
    ...
    <div className="min-h-screen bg-gradient-to-br from-slate-900 via-purple-900 to-slate-900 p-6">
      ...
      <div className="max-w-7xl mx-auto">
        ...
        <div className="text-center mb-8">
          <h1 className="text-4xl font-bold text-white mb-4 casino-neon">🎯 Classic BINGO</h1>
          <p className="text-white/80 text-lg">Match 5 numbers in a row, column, or diagonal to win!</p>
        </div>
          ...
          <div className="flex flex-wrap gap-2">
            {calledNumbers.map((num, index) => (
              <Badge 
                key={index}
                variant="secondary"
                className="bg-green-600 text-white text-lg px-3 py-2"
              >
                {num}
              </Badge>
            ))}
